...
contract Task {
    ...
    require(msg.sender == marketplaceAddress, "Only the marketplace can assign workers.");

    TaskInfo storage task = tasks[taskId];
    require(task.status == TaskStatus.Created || task.status == TaskStatus.TrainingCompleted, "Task must be in Created or TrainingCompleted status.");
    task.status = task.taskType == TaskType.Training ? TaskStatus.TrainingAssigned : TaskStatus.ValidationAssigned;
    task.worker = worker;
    emit TaskAssigned(taskId, worker);
    }
